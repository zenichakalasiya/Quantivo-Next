// Vendored verbatim from the sibling image-slot.js next to Quantivo.dc.html.
// NOT reimplemented: outside the Claude Design host window.omelette is absent, so
// editable resolves false and the element renders exactly the read-only placeholder
// the deployed site shows. Reproducing that by hand would only risk drifting from it.
// The .image-slots.state.json fetch 404s here and is swallowed by its own .catch().
